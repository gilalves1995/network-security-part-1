...
public class MChatCliente extends JFrame implements MulticastChatEventListener
{
	...
	public void join(String username, InetAddress group, int port, 
					 int ttl, STGCProtocol stgc) throws IOException {
		...
		chat = new MulticastChat(username, group, port, ttl, this, stgc);
	} 
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Added by us
			STGCProtocol stgc = new STGCProtocol();
			Scanner in = new Scanner(System.in);
			System.out.println("Insert password:");
			String password = in.nextLine();
			in.close();
						
			if(stgc.initializeKeystore(password) == null) {
				System.exit(1);
			}
			stgc.securityConfig(group.toString());
			...
			frame.join(username, group, port, ttl, stgc);
		} catch (Throwable e) {
